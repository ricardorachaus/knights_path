...
public class Scene3 : SKScene {
    ...
    public override init(size: CGSize) {
        ...
        self.spaceship = SKSpriteNode(imageNamed: "Objects/Spaceship.png")
        self.spaceship?.setScale(1.5)
        super.init(size: size)
        
        let position = CGPoint (x: 300 / 2, y: 290 / 2)
        self.spaceship?.position = position
        self.spaceship?.zPosition = 10
        self.spaceship?.name = "secretScene"
    ...
    func changeScene (location: CGPoint) {
        if (location.x >= 220) {
            let nextScene = Scene4 (size: size)
...
extension Scene3 {
    public override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if node?.name == "secretScene"{
            let alternativeScene = Space (size: self.size)
            let transition = SKTransition.fade(withDuration: 0.5)
