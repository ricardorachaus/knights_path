...
public class Alien : GameObject {
    var alienTexture = SKTexture()
    
    init(position: CGPoint) {
        let texture = SKTexture (imageNamed: "Alien/Alien")
        texture.filteringMode = .nearest
        alienTexture = texture
        
        super.init(position: position, texture: alienTexture)
        setupPhysicsBody()
        self.run(animateAlien())
    ...
    func animateAlien () -> SKAction {
        let animate = SKAction.animate(with: [alienTexture], timePerFrame: 0.1)
